...
func TestBackup2B(t *testing.T) {
	...
	DPrintf("group: %d, %d", (leader1+0)%servers, (leader1+1)%servers)
	...
	DPrintf("group: %d, %d, %d", (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	...
	DPrintf("second time, lots more commands that won't commit")
	...
	cfg.connect(other)
	DPrintf("group: %d, %d, %d", (leader1+0)%servers, (leader1+1)%servers, other)
	...
	DPrintf("second time, lots of successful commands to new group")
...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		if leader != -1 && (rand.Int()%1000) < int(RaftElectionTimeout/time.Millisecond)/2 {
			DPrintf("disonnect leader %d", leader)
